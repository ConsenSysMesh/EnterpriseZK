package json

import (
	"github.com/consensys/gnark/std/accumulator/merkle"
)

type Data struct {
	Leaf                string             `json:"Leaf" binding:"required"`
	Latitude            float64            `json:"Latitude" binding:"required"`
	Longitude           float64            `json:"Longitude" binding:"required"`
	ApprovedLatitude    float64            `json:"ApprovedLatitude" binding:"required"`
	ApprovedLongitude   float64            `json:"ApprovedLongitude" binding:"required"`
	MaxAllowedDistance  float64            `json:"MaxAllowedDistance" binding:"required"`
	MerkleProofReceiver merkle.MerkleProof `json:"MerkleProofReceiver" binding:"required"`
}

type PublicData struct {
	Leaf               string  `json:"Leaf" binding:"required"`
	ApprovedLatitude   float64 `json:"ApprovedLatitude" binding:"required"`
	ApprovedLongitude  float64 `json:"ApprovedLongitude" binding:"required"`
	MaxAllowedDistance float64 `json:"MaxAllowedDistance" binding:"required"`
	RootHash           string  `json:"RootHash" binding:"required"`
	Proof              string  `json:"Proof" binding:"required"`
	VerifyingKey       string  `json:"VerifyingKey" binding:"required"`
}

type Tree struct {
	Leaves    []string `json:"Leaves" binding:"required"`
	Depth     int      `json:"Depth" binding:"required"`
	LeafIndex int      `json:"LeafIndex"`
}

type ProveResult struct {
	Proof        string `json:"Proof" example:""`
	VerifyingKey string `json:"VerifyingKey" example:""`
}

type VerifyResult struct {
	Message string `json:"message" example:"Valid Proof"`
}

type BadProveResult struct {
	Message string `json:"message" example:"constraint #XXXX is not satisfied"`
}

type BadVerifyResult struct {
	Message string `json:"message" example:"pairing doesn't match"`
}
